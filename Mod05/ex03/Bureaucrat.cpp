...
/*   Updated: 2020/04/19 19:57:18 by henri            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::executeForm(Form const &f) const {
	if (f.getSigned())
		std::cout << this->name << " cant execute the form " << f.getName() << " because it isn't signed" << std::endl;
	else if (f.getSignGrade() < this->grade)
		std::cout << this->name << " cant execute the form " << f.getName() << " because its grade is too low" << std::endl;
	else
		std::cout << this->name << " execute " << f.getName() << std::endl;
	f.execute(*this);
}
